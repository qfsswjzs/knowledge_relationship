...
setwd("/Users/wujie/Documents/Projects/Pycharm/graduation_project/knowledge dataset/alpha-parameter-evaluation")
source("utils.R")
...
for (j in n_ratios) {
  ...
  for (n in 1:N_realizations) {
    ...
    for (i in alpha_size) {
      ...
      graph <- learn_k_component_graph(S, w0 = "qp", k = 4, ub = 2*N,
                                  alpha = alpha_set[i], beta = 10*N, maxiter = 100000)
      print(graph$convergence)
      print(graph$lambda)
      alpha_rel_err[i, j] <- alpha_rel_err[i, j] + relative_error(Ltrue, graph$Laplacian)
